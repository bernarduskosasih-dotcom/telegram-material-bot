...
async function sendDailyReport() {
  ...
  // === BANGUN PESAN REPORT ===
  let message = `📋 <b>LAPORAN HARIAN APT</b>\n`;
  message += `📅 Tanggal: <b>${yesterday}</b>\n`;
  message += `⏰ Report dikirim: ${new Date().toLocaleString("id-ID", { timeZone: "Asia/Jakarta" })}\n`;
  message += `${"─".repeat(30)}\n\n`;

  if (!data || data.length === 0) {
    message += `ℹ️ Tidak ada pemakaian bahan kemarin.\n\n`;
  } else {
    // Group by project
    const byProject = {};
    data.forEach((row) => {
    ...
    message += `🏗️ <b>PEMAKAIAN PER PROJECT</b>\n\n`;

    for (const [projName, materials] of Object.entries(byProject)) {
      message += `▸ <b>${projName}</b>\n`;
      for (const [matName, info] of Object.entries(materials)) {
        message += `   📦 ${matName}: ${info.total.toFixed(1)} ${info.unit}\n`;
      }
      message += "\n";
    ...
    data.forEach((row) => {
      if (!totalByMaterial[row.material_name]) totalByMaterial[row.material_name] = { total: 0, unit: row.unit };
      totalByMaterial[row.material_name].total += parseFloat(row.quantity);
    });

    message += `📊 <b>TOTAL PEMAKAIAN</b>\n`;
    for (const [name, info] of Object.entries(totalByMaterial)) {
      message += `   ${name}: ${info.total.toFixed(1)} ${info.unit}\n`;
    }
    message += "\n";
  }
