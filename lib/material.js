const XLSX = require("xlsx");
const { supabase } = require("./supabase");
const {
  sendMessage,
  sendInlineKeyboard,
  answerCallback,
  sendDocument,
} = require("./telegram");
const { SUPPLIERS, MATERIAL_LIST, getMaterialsBySupplier, getMaterialById } = require("./data");

const MASTER_CHAT_ID = process.env.MASTER_CHAT_ID;

// ============================================================
// Session management
// ============================================================
async function saveSession(chatId, sessionData) {
  const { error } = await supabase.from("sessions").upsert(
    { chat_id: String(chatId), data: sessionData, updated_at: new Date().toISOString() },
    { onConflict: "chat_id" }
  );
  if (error) console.error("Save session error:", error);
}

async function getSession(chatId) {
  const { data, error } = await supabase
    .from("sessions").select("data").eq("chat_id", String(chatId)).single();
  if (error || !data) return null;
  return data.data;
}

async function clearSession(chatId) {
  await supabase.from("sessions").delete().eq("chat_id", String(chatId));
}

function isMaster(chatId) {
  return String(chatId) === String(MASTER_CHAT_ID);
}

// ============================================================
// Menu utama
// ============================================================
async function showMainMenu(chatId, userName) {
  const buttons = [
    [
      { text: "📦 Lapor Pemakaian", callback_data: "menu_laporan" },
      { text: "📊 Status Hari Ini", callback_data: "menu_status" },
    ],
    [
      { text: "📋 Riwayat 7 Hari", callback_data: "menu_riwayat" },
      { text: "📦 Cek Stok", callback_data: "menu_stok" },
    ],
    [
      { text: "❓ Bantuan", callback_data: "menu_bantuan" },
    ],
  ];

  // Barang masuk - semua user bisa akses
  buttons.push([
    { text: "🚚 Barang Masuk", callback_data: "menu_restock" },
  ]);

  // Menu master only
  if (isMaster(chatId)) {
    buttons.push([
      { text: "🗑️ Hapus Data", callback_data: "menu_delete" },
      { text: "📤 Kirim Report", callback_data: "menu_send_report" },
    ]);
    buttons.push([
      { text: "🔧 Adjustment Stok", callback_data: "menu_adjust" },
      { text: "📈 Report Project", callback_data: "menu_report_project" },
    ]);
    buttons.push([
      { text: "📥 Upload Excel Adjustment", callback_data: "menu_adjust_excel" },
    ]);
    buttons.push([
      { text: "⚙️ Kelola Project", callback_data: "menu_project" },
    ]);
  }

  await sendInlineKeyboard(
    chatId,
    `Halo <b>${userName}</b>! 👋\n\nBot Laporan Pemakaian Bahan APT.\nPilih menu:`,
    buttons
  );
}

// ============================================================
// FLOW LAPORAN: Step 1 - Pilih supplier
// ============================================================
async function startReport(chatId) {
  const buttons = SUPPLIERS.map((s) => [
    { text: s.name, callback_data: `supplier_${s.id}` },
  ]);
  buttons.push([{ text: "❌ Batal", callback_data: "menu_batal" }]);

  await sendInlineKeyboard(
    chatId,
    "📦 <b>Lapor Pemakaian Bahan</b>\n\nPilih supplier:",
    buttons
  );
}

// ============================================================
// Step 2 - Pilih bahan (dari supplier yang dipilih)
// ============================================================
async function showMaterials(chatId, supplierId, page = 0, mode = "usage") {
  const materials = getMaterialsBySupplier(supplierId);
  const PAGE_SIZE = 8;
  const start = page * PAGE_SIZE;
  const end = start + PAGE_SIZE;
  const pageMaterials = materials.slice(start, end);

  const prefix = mode === "restock" ? "rmat_" : "mat_";

  const buttons = [];
  for (let i = 0; i < pageMaterials.length; i += 2) {
    const row = [{ text: pageMaterials[i].name, callback_data: `${prefix}${pageMaterials[i].id}` }];
    if (pageMaterials[i + 1]) {
      row.push({ text: pageMaterials[i + 1].name, callback_data: `${prefix}${pageMaterials[i + 1].id}` });
    }
    buttons.push(row);
  }

  // Navigasi halaman
  const pagePrefix = mode === "restock" ? "rpage_" : "page_";
  const navRow = [];
  if (page > 0) navRow.push({ text: "⬅️ Sebelumnya", callback_data: `${pagePrefix}${supplierId}_${page - 1}` });
  if (end < materials.length) navRow.push({ text: "Selanjutnya ➡️", callback_data: `${pagePrefix}${supplierId}_${page + 1}` });
  if (navRow.length) buttons.push(navRow);

  const backMenu = mode === "restock" ? "menu_restock" : "menu_laporan";
  buttons.push([
    { text: "🔙 Pilih Supplier", callback_data: backMenu },
    { text: "❌ Batal", callback_data: "menu_batal" },
  ]);

  const supplier = SUPPLIERS.find((s) => s.id === supplierId);
  const totalPages = Math.ceil(materials.length / PAGE_SIZE);
  const title = mode === "restock" ? "🚚 <b>Barang Masuk</b>" : "📦 <b>Pemakaian</b>";
  await sendInlineKeyboard(
    chatId,
    `${title} — <b>${supplier.name}</b>\nPilih bahan: (Hal ${page + 1}/${totalPages})`,
    buttons
  );
}

// ============================================================
// Step 3 - Input jumlah
// ============================================================
async function askQuantity(chatId, materialId) {
  const material = getMaterialById(materialId);
  if (!material) {
    await sendMessage(chatId, "❌ Bahan tidak ditemukan.");
    return;
  }

  // Hitung stok saat ini
  const { data } = await supabase
    .from("material_usage").select("quantity").eq("material_id", materialId);
  const totalUsed = data ? data.reduce((sum, r) => sum + parseFloat(r.quantity), 0) : 0;
  const currentStock = material.initial_stock - totalUsed;

  // Cek apakah stok sudah 0 atau kurang
  if (currentStock <= 0) {
    await sendMessage(chatId,
      `⛔ <b>${material.name}</b> stok habis! (${currentStock.toFixed(1)} ${material.unit})\n\n` +
      `Lakukan restock terlebih dahulu sebelum bisa input pemakaian.`
    );
    return;
  }

  await saveSession(chatId, {
    step: "input_quantity",
    material_id: materialId,
    material_name: material.name,
    material_unit: material.unit,
    supplier: material.supplier,
    current_stock: currentStock,
  });

  await sendMessage(
    chatId,
    `📏 Berapa <b>${material.unit}</b> <b>${material.name}</b> yang dipakai?\n\n` +
    `📊 Stok tersedia: <b>${currentStock.toFixed(1)} ${material.unit}</b>\n\n` +
    `Ketik angka (contoh: <code>12.5</code>)`
  );
}

// ============================================================
// Step 4 - Input tanggal
// ============================================================
async function askDate(chatId, quantity) {
  const session = await getSession(chatId);
  if (!session) return;

  // Cek apakah jumlah melebihi stok
  const currentStock = session.current_stock || 0;
  if (quantity > currentStock) {
    await sendMessage(chatId,
      `⛔ Jumlah melebihi stok!\n\n` +
      `📦 <b>${session.material_name}</b>\n` +
      `📊 Stok tersedia: <b>${currentStock.toFixed(1)} ${session.material_unit}</b>\n` +
      `❌ Input kamu: <b>${quantity} ${session.material_unit}</b>\n\n` +
      `Masukkan angka yang tidak melebihi stok, atau lakukan restock dulu.\n` +
      `Ketik angka baru atau /batal untuk membatalkan.`
    );
    return; // Session tetap di input_quantity, user bisa ketik ulang
  }

  session.step = "input_date";
  session.quantity = quantity;
  await saveSession(chatId, session);

  const today = getJakartaDate();

  const buttons = [
    [{ text: `📅 Hari ini (${today})`, callback_data: `date_${today}` }],
    [{ text: `📅 Kemarin (${getDateOffset(-1)})`, callback_data: `date_${getDateOffset(-1)}` }],
    [{ text: "✏️ Ketik tanggal lain", callback_data: "date_custom" }],
  ];

  await sendInlineKeyboard(chatId, "📅 Tanggal pemakaian:", buttons);
}

// ============================================================
// Step 5 - Pilih project
// ============================================================
async function askProject(chatId, dateStr) {
  const session = await getSession(chatId);
  if (!session) return;

  session.step = "input_project";
  session.report_date = dateStr;
  await saveSession(chatId, session);

  const { data: projects, error } = await supabase
    .from("projects").select("*").eq("is_active", true).order("name");

  if (error || !projects || projects.length === 0) {
    await sendMessage(chatId, "⚠️ Belum ada project aktif. Minta master untuk menambah project.\nKetik /start untuk menu.");
    await clearSession(chatId);
    return;
  }

  const buttons = projects.map((p) => [
    { text: p.name, callback_data: `proj_${p.id}` },
  ]);
  buttons.push([{ text: "❌ Batal", callback_data: "menu_batal" }]);

  await sendInlineKeyboard(chatId, "🏗️ Untuk project apa?", buttons);
}

// ============================================================
// Step 6 - Simpan & cek stok
// ============================================================
async function confirmAndSave(chatId, projectId, userId, userName) {
  const session = await getSession(chatId);
  if (!session) return;

  if (!session.material_id || !session.quantity || !session.report_date) {
    await sendMessage(
      chatId,
      "⚠️ Sesi input sudah kedaluwarsa atau kepakai untuk aksi lain di tengah jalan. Silakan ulangi dari awal: ketik /laporan"
    );
    await clearSession(chatId);
    return;
  }

  const { data: project } = await supabase
    .from("projects").select("name").eq("id", projectId).single();
  const projectName = project ? project.name : "-";

  const { error } = await supabase.from("material_usage").insert({
    material_id: session.material_id,
    material_name: session.material_name,
    quantity: session.quantity,
    unit: session.material_unit,
    supplier: session.supplier,
    project_id: projectId,
    project_name: projectName,
    order_note: projectName,
    operator_id: String(userId),
    operator_name: userName,
    reported_at: new Date().toISOString(),
    report_date: session.report_date,
  });

  if (error) {
    console.error("Insert error:", error);
    await sendMessage(chatId, "❌ Gagal menyimpan. Error: " + error.message);
    await clearSession(chatId);
    return;
  }

  const summary =
    `✅ <b>Laporan Tersimpan!</b>\n\n` +
    `📦 Bahan: <b>${session.material_name}</b>\n` +
    `📏 Jumlah: <b>${session.quantity} ${session.material_unit}</b>\n` +
    `📅 Tanggal: <b>${session.report_date}</b>\n` +
    `🏗️ Project: <b>${projectName}</b>\n` +
    `👤 Operator: <b>${userName}</b>`;

  await sendMessage(chatId, summary);
  await checkStockAlert(session.material_id, session.material_name);

  await sendInlineKeyboard(chatId, "Mau input lagi?", [
    [
      { text: "✅ Ya, input lagi", callback_data: "menu_laporan" },
      { text: "🏠 Menu Utama", callback_data: "menu_utama" },
    ],
  ]);

  await clearSession(chatId);
}

// ============================================================
// FITUR BARU 1: BARANG MASUK (Restock) - Master only
// ============================================================
async function startRestock(chatId) {
  const buttons = SUPPLIERS.map((s) => [
    { text: s.name, callback_data: `rsupplier_${s.id}` },
  ]);
  buttons.push([{ text: "❌ Batal", callback_data: "menu_batal" }]);

  await sendInlineKeyboard(
    chatId,
    "🚚 <b>Barang Masuk dari Supplier</b>\n\nPilih supplier:",
    buttons
  );
}

async function askRestockQuantity(chatId, materialId) {
  const material = getMaterialById(materialId);
  if (!material) {
    await sendMessage(chatId, "❌ Bahan tidak ditemukan.");
    return;
  }

  await saveSession(chatId, {
    step: "input_restock_qty",
    material_id: materialId,
    material_name: material.name,
    material_unit: material.unit,
    supplier: material.supplier,
  });

  await sendMessage(
    chatId,
    `🚚 Berapa <b>${material.unit}</b> <b>${material.name}</b> yang masuk?\n\nKetik angka (contoh: <code>225</code>)`
  );
}

async function confirmRestock(chatId, quantity, userName) {
  const session = await getSession(chatId);
  if (!session) return;

  if (!session.material_id) {
    await sendMessage(
      chatId,
      "⚠️ Sesi input sudah kedaluwarsa atau kepakai untuk aksi lain di tengah jalan. Silakan ulangi dari awal."
    );
    await clearSession(chatId);
    return;
  }

  // Simpan sebagai quantity negatif (mengurangi "total pemakaian" = menambah stok)
  const { error } = await supabase.from("material_usage").insert({
    material_id: session.material_id,
    material_name: session.material_name,
    quantity: -quantity, // negatif = restock
    unit: session.material_unit,
    supplier: session.supplier,
    project_id: null,
    project_name: "RESTOCK",
    order_note: "Barang masuk dari supplier",
    operator_id: String(MASTER_CHAT_ID),
    operator_name: userName,
    reported_at: new Date().toISOString(),
    report_date: getJakartaDate(),
  });

  if (error) {
    await sendMessage(chatId, "❌ Gagal menyimpan. Error: " + error.message);
  } else {
    // Hitung stok terbaru
    const material = getMaterialById(session.material_id);
    const { data } = await supabase
      .from("material_usage").select("quantity").eq("material_id", session.material_id);
    const totalUsed = data ? data.reduce((sum, r) => sum + parseFloat(r.quantity), 0) : 0;
    const currentStock = material.initial_stock - totalUsed;

    await sendMessage(chatId,
      `✅ <b>Barang Masuk Tercatat!</b>\n\n` +
      `📦 Bahan: <b>${session.material_name}</b>\n` +
      `🚚 Jumlah masuk: <b>+${quantity} ${session.material_unit}</b>\n` +
      `📊 Stok sekarang: <b>${currentStock.toFixed(1)} ${session.material_unit}</b>\n` +
      `📅 Tanggal: <b>${getJakartaDate()}</b>`
    );
  }

  await sendInlineKeyboard(chatId, "Lanjut?", [
    [
      { text: "🚚 Input barang masuk lagi", callback_data: "menu_restock" },
      { text: "🏠 Menu Utama", callback_data: "menu_utama" },
    ],
  ]);

  await clearSession(chatId);
}

// ============================================================
// FITUR BARU 2: HAPUS DATA - Master only
// ============================================================
async function showRecentEntries(chatId) {
  const { data, error } = await supabase
    .from("material_usage")
    .select("*")
    .neq("project_name", "RESTOCK")
    .order("reported_at", { ascending: false })
    .limit(10);

  if (error || !data || data.length === 0) {
    await sendMessage(chatId, "🗑️ Tidak ada data untuk dihapus.");
    return;
  }

  let message = "🗑️ <b>Pilih data yang mau dihapus:</b>\n\n";

  const buttons = [];
  data.forEach((row, i) => {
    const time = new Date(row.reported_at).toLocaleTimeString("id-ID", {
      hour: "2-digit", minute: "2-digit", timeZone: "Asia/Jakarta",
    });
    message += `${i + 1}. ${row.material_name} — ${row.quantity} ${row.unit}\n`;
    message += `   📅 ${row.report_date} | 🏗️ ${row.project_name} | 👤 ${row.operator_name} (${time})\n\n`;
    buttons.push([{ text: `🗑️ Hapus #${i + 1}: ${row.material_name} ${row.quantity}${row.unit}`, callback_data: `del_${row.id}` }]);
  });

  buttons.push([{ text: "🏠 Menu Utama", callback_data: "menu_utama" }]);

  await sendInlineKeyboard(chatId, message, buttons);
}

async function deleteEntry(chatId, entryId) {
  // Ambil data dulu sebelum hapus
  const { data: entry } = await supabase
    .from("material_usage").select("*").eq("id", entryId).single();

  if (!entry) {
    await sendMessage(chatId, "❌ Data tidak ditemukan.");
    return;
  }

  const { error } = await supabase.from("material_usage").delete().eq("id", entryId);

  if (error) {
    await sendMessage(chatId, "❌ Gagal menghapus: " + error.message);
  } else {
    await sendMessage(chatId,
      `✅ <b>Data dihapus!</b>\n\n` +
      `📦 ${entry.material_name} — ${entry.quantity} ${entry.unit}\n` +
      `📅 ${entry.report_date} | 👤 ${entry.operator_name}`
    );
  }

  // Tampilkan daftar lagi
  await showRecentEntries(chatId);
}

// ============================================================
// FITUR BARU 3: REPORT PER PROJECT - Master only
// ============================================================
async function showProjectReport(chatId) {
  const { data: projects } = await supabase
    .from("projects").select("*").eq("is_active", true).order("name");

  if (!projects || projects.length === 0) {
    await sendMessage(chatId, "Belum ada project aktif.\n\n<i>Project yang sudah dinonaktifkan tidak muncul di sini — laporannya sudah dikirim otomatis via Excel saat dinonaktifkan.</i>");
    return;
  }

  const buttons = projects.map((p) => [
    { text: `📈 ${p.name}`, callback_data: `rproj_${p.id}` },
  ]);
  buttons.push([{ text: "📈 Semua Project (Ringkasan)", callback_data: "rproj_all" }]);
  buttons.push([{ text: "🏠 Menu Utama", callback_data: "menu_utama" }]);

  await sendInlineKeyboard(
    chatId,
    "📈 <b>Report Pemakaian per Project</b>\n\nPilih project (hanya project aktif):",
    buttons
  );
}

async function showProjectDetail(chatId, projectId) {
  let query = supabase.from("material_usage").select("*").neq("project_name", "RESTOCK");

  if (projectId !== "all") {
    query = query.eq("project_id", parseInt(projectId));
  }

  const { data, error } = await query.order("material_name");

  if (error || !data || data.length === 0) {
    await sendMessage(chatId, "📈 Belum ada data untuk project ini.");
    return;
  }

  if (projectId === "all") {
    // Ringkasan semua project
    let message = "📈 <b>Ringkasan Semua Project</b>\n\n";
    const byProject = {};
    data.forEach((row) => {
      const pn = row.project_name || "-";
      if (!byProject[pn]) byProject[pn] = {};
      if (!byProject[pn][row.material_name]) byProject[pn][row.material_name] = { total: 0, unit: row.unit };
      byProject[pn][row.material_name].total += parseFloat(row.quantity);
    });

    for (const [projName, materials] of Object.entries(byProject)) {
      message += `🏗️ <b>${projName}</b>\n`;
      for (const [matName, info] of Object.entries(materials)) {
        message += `   📦 ${matName}: ${info.total.toFixed(1)} ${info.unit}\n`;
      }
      message += "\n";
    }

    await sendMessage(chatId, message);
  } else {
    // Detail satu project
    const projectName = data[0]?.project_name || "Unknown";
    let message = `📈 <b>Report: ${projectName}</b>\n\n`;

    const byMaterial = {};
    data.forEach((row) => {
      if (!byMaterial[row.material_name]) byMaterial[row.material_name] = { total: 0, unit: row.unit, entries: [] };
      byMaterial[row.material_name].total += parseFloat(row.quantity);
      byMaterial[row.material_name].entries.push(row);
    });

    for (const [name, info] of Object.entries(byMaterial)) {
      message += `📦 <b>${name}</b>: ${info.total.toFixed(1)} ${info.unit}\n`;
      info.entries.forEach((e) => {
        message += `   └ ${e.report_date}: ${e.quantity} ${e.unit} (${e.operator_name})\n`;
      });
      message += "\n";
    }

    const totalEntries = data.length;
    message += `Total ${totalEntries} catatan pemakaian.`;

    await sendMessage(chatId, message);
  }

  await sendInlineKeyboard(chatId, "Lihat project lain?", [
    [
      { text: "📈 Project lain", callback_data: "menu_report_project" },
      { text: "🏠 Menu Utama", callback_data: "menu_utama" },
    ],
  ]);
}

// ============================================================
// Cek stok & kirim alert ke master
// ============================================================
async function checkStockAlert(materialId, materialName) {
  if (!MASTER_CHAT_ID) return;

  const material = getMaterialById(materialId);
  if (!material) return;

  const { data, error } = await supabase
    .from("material_usage").select("quantity").eq("material_id", materialId);

  if (error || !data) return;

  const totalUsed = data.reduce((sum, row) => sum + parseFloat(row.quantity), 0);
  const currentStock = material.initial_stock - totalUsed;

  if (currentStock <= material.min_stock) {
    const alert =
      `🚨 <b>ALERT STOK RENDAH!</b>\n\n` +
      `📦 <b>${materialName}</b>\n` +
      `📊 Stok saat ini: <b>${currentStock.toFixed(1)} ${material.unit}</b>\n` +
      `⚠️ Minimum stok: <b>${material.min_stock} ${material.unit}</b>\n` +
      `📉 Total terpakai: <b>${totalUsed.toFixed(1)} ${material.unit}</b>\n\n` +
      `Segera order ke supplier!`;

    await sendMessage(MASTER_CHAT_ID, alert);
  }
}

// ============================================================
// Status hari ini
// ============================================================
async function showTodayStatus(chatId) {
  const today = getJakartaDate();

  const { data, error } = await supabase
    .from("material_usage").select("*")
    .eq("report_date", today)
    .neq("project_name", "RESTOCK")
    .order("reported_at", { ascending: false });

  if (error || !data || data.length === 0) {
    await sendMessage(chatId, `📊 <b>Status Pemakaian</b> (${today})\n\nBelum ada laporan hari ini.`);
    return;
  }

  let message = `📊 <b>Status Pemakaian</b> (${today})\n\n`;

  const grouped = {};
  data.forEach((row) => {
    if (!grouped[row.material_name]) grouped[row.material_name] = { total: 0, unit: row.unit, entries: [] };
    grouped[row.material_name].total += parseFloat(row.quantity);
    grouped[row.material_name].entries.push(row);
  });

  for (const [name, info] of Object.entries(grouped)) {
    message += `📦 <b>${name}</b>: ${info.total} ${info.unit}\n`;
    info.entries.forEach((e) => {
      const time = new Date(e.reported_at).toLocaleTimeString("id-ID", {
        hour: "2-digit", minute: "2-digit", timeZone: "Asia/Jakarta",
      });
      message += `   └ ${e.quantity} ${e.unit} — ${e.project_name || e.order_note} (${time}, ${e.operator_name})\n`;
    });
    message += "\n";
  }

  message += `Total ${data.length} laporan.`;
  await sendMessage(chatId, message);
}

// ============================================================
// Riwayat 7 hari
// ============================================================
async function showHistory(chatId) {
  const today = new Date();
  const weekAgo = new Date(today);
  weekAgo.setDate(weekAgo.getDate() - 7);

  const { data, error } = await supabase
    .from("material_usage")
    .select("report_date, material_name, quantity, unit, project_name")
    .neq("project_name", "RESTOCK")
    .gte("report_date", weekAgo.toISOString().split("T")[0])
    .order("report_date", { ascending: false });

  if (error || !data || data.length === 0) {
    await sendMessage(chatId, "📋 Tidak ada data dalam 7 hari terakhir.");
    return;
  }

  let message = `📋 <b>Riwayat 7 Hari Terakhir</b>\n\n`;

  const byDate = {};
  data.forEach((row) => {
    if (!byDate[row.report_date]) byDate[row.report_date] = [];
    byDate[row.report_date].push(row);
  });

  for (const [date, entries] of Object.entries(byDate)) {
    message += `📅 <b>${date}</b>\n`;
    const byMaterial = {};
    entries.forEach((e) => {
      if (!byMaterial[e.material_name]) byMaterial[e.material_name] = { total: 0, unit: e.unit };
      byMaterial[e.material_name].total += parseFloat(e.quantity);
    });
    for (const [name, info] of Object.entries(byMaterial)) {
      message += `   ${name}: ${info.total} ${info.unit}\n`;
    }
    message += "\n";
  }

  await sendMessage(chatId, message);
}

// ============================================================
// Cek stok semua bahan
// ============================================================
// Bangun teks status stok (dipakai di menu "Cek Stok" & report harian ke grup)
function buildStockStatusMessage(usageMap) {
  let message = `📦 <b>Status Stok Bahan</b>\n\n`;
  let lowStockCount = 0;

  for (const supplier of SUPPLIERS) {
    const materials = getMaterialsBySupplier(supplier.id);
    message += `<b>🏭 ${supplier.name}</b>\n`;

    materials.forEach((m) => {
      const used = usageMap[m.id] || 0;
      const current = m.initial_stock - used;
      const isLow = current <= m.min_stock;
      const icon = isLow ? "🔴" : "🟢";
      if (isLow) lowStockCount++;
      message += `${icon} ${m.name}: ${current.toFixed(1)} ${m.unit}`;
      if (isLow) message += ` ⚠️`;
      message += "\n";
    });
    message += "\n";
  }

  if (lowStockCount > 0) {
    message += `\n⚠️ <b>${lowStockCount} bahan di bawah minimum stok!</b>`;
  }

  return message;
}

async function showStockStatus(chatId) {
  const { data } = await supabase.from("material_usage").select("material_id, quantity");

  const usageMap = {};
  if (data) {
    data.forEach((row) => {
      if (!usageMap[row.material_id]) usageMap[row.material_id] = 0;
      usageMap[row.material_id] += parseFloat(row.quantity);
    });
  }

  await sendMessage(chatId, buildStockStatusMessage(usageMap));
}

// ============================================================
// Project management (Master only)
// ============================================================
async function showProjectMenu(chatId) {
  const { data: projects } = await supabase
    .from("projects").select("*").order("name");

  let message = "⚙️ <b>Kelola Project</b>\n\n";

  if (projects && projects.length > 0) {
    projects.forEach((p) => {
      message += `${p.is_active ? "🟢" : "🔴"} ${p.name}\n`;
    });
  } else {
    message += "Belum ada project.\n";
  }

  const buttons = [
    [{ text: "➕ Tambah Project", callback_data: "proj_add" }],
    [{ text: "🏠 Menu Utama", callback_data: "menu_utama" }],
  ];

  if (projects && projects.length > 0) {
    buttons.splice(1, 0, [{ text: "✅ Aktifkan/Nonaktifkan", callback_data: "proj_toggle" }]);
    buttons.splice(2, 0, [{ text: "🔀 Pindahkan Data Project", callback_data: "proj_transfer" }]);
  }

  await sendInlineKeyboard(chatId, message, buttons);
}

async function startAddProject(chatId) {
  await saveSession(chatId, { step: "add_project" });
  await sendMessage(chatId, "✏️ Ketik nama project baru:");
}

async function saveProject(chatId, projectName) {
  const { error } = await supabase.from("projects").insert({ name: projectName, is_active: true });
  if (error) {
    await sendMessage(chatId, "❌ Gagal: " + error.message);
  } else {
    await sendMessage(chatId, `✅ Project <b>${projectName}</b> ditambahkan!`);
  }
  await clearSession(chatId);
  await showProjectMenu(chatId);
}

async function showToggleProjects(chatId) {
  const { data: projects } = await supabase.from("projects").select("*").order("name");
  if (!projects || projects.length === 0) {
    await sendMessage(chatId, "Belum ada project.");
    return;
  }

  const buttons = projects.map((p) => [
    { text: `${p.is_active ? "🟢" : "🔴"} ${p.name}`, callback_data: `toggle_${p.id}` },
  ]);
  buttons.push([{ text: "🔙 Kembali", callback_data: "menu_project" }]);

  await sendInlineKeyboard(chatId, "Klik project untuk aktifkan/nonaktifkan:", buttons);
}

async function toggleProject(chatId, projectId) {
  const { data: project } = await supabase
    .from("projects").select("*").eq("id", projectId).single();
  if (!project) return;

  const wasActive = project.is_active;
  await supabase.from("projects").update({ is_active: !wasActive }).eq("id", projectId);
  const status = !wasActive ? "diaktifkan ✅" : "dinonaktifkan 🔴";
  await sendMessage(chatId, `Project <b>${project.name}</b> ${status}`);

  // Baru dinonaktifkan (bukan diaktifkan) → kirim report Excel pemakaian bahan project ini
  if (wasActive) {
    await sendMessage(chatId, "📊 Menyiapkan report Excel pemakaian bahan...");
    try {
      const { buffer, count } = await generateProjectUsageExcel(projectId);
      if (count === 0) {
        await sendMessage(chatId, `ℹ️ Belum ada data pemakaian tercatat untuk project <b>${project.name}</b>.`);
      } else {
        const safeName = project.name.replace(/[^a-zA-Z0-9]+/g, "_");
        const filename = `Laporan_${safeName}_${getJakartaDate()}.xlsx`;
        await sendDocument(chatId, buffer, filename, `📊 Laporan pemakaian bahan — <b>${project.name}</b>`);
      }
    } catch (err) {
      console.error("Generate project excel error:", err);
      await sendMessage(chatId, "⚠️ Gagal membuat report Excel: " + err.message);
    }
  }

  await showToggleProjects(chatId);
}

// ============================================================
// Transfer Data Project (Master only)
// Pindahkan semua data pemakaian dari 1 project (misal "Stok Pasir 2026"
// yang dipakai sebagai penampung sementara) ke project lain (project asli,
// bisa yang sudah ada atau bikin baru), tanpa menghapus riwayat.
// ============================================================
async function startTransferProject(chatId) {
  const { data: projects } = await supabase
    .from("projects").select("*").eq("is_active", true).order("name");

  if (!projects || projects.length === 0) {
    await sendMessage(chatId, "Belum ada project aktif.");
    return;
  }

  const buttons = projects.map((p) => [{ text: p.name, callback_data: `transfersrc_${p.id}` }]);
  buttons.push([{ text: "🔙 Kembali", callback_data: "menu_project" }]);

  await sendInlineKeyboard(
    chatId,
    "🔀 <b>Pindahkan Data Project</b>\n\nPilih project <b>sumber</b> (data yang mau dipindahkan), misal project penampung sementara seperti \"Stok Pasir 2026\":",
    buttons
  );
}

async function showTransferTargets(chatId, sourceId) {
  const { data: source } = await supabase.from("projects").select("*").eq("id", sourceId).single();
  if (!source) return sendMessage(chatId, "❌ Project tidak ditemukan.");

  const { data: projects } = await supabase
    .from("projects").select("*").eq("is_active", true).neq("id", sourceId).order("name");

  const buttons = (projects || []).map((p) => [
    { text: p.name, callback_data: `transfertgt_${sourceId}_${p.id}` },
  ]);
  buttons.push([{ text: "➕ Project Baru", callback_data: `transfernew_${sourceId}` }]);
  buttons.push([{ text: "🔙 Kembali", callback_data: "proj_transfer" }]);

  await sendInlineKeyboard(
    chatId,
    `🔀 Pindahkan data dari <b>${source.name}</b>\n\nPilih project <b>tujuan</b>:`,
    buttons
  );
}

async function startTransferNewProject(chatId, sourceId) {
  await saveSession(chatId, { step: "input_transfer_new_name", transferSourceId: sourceId });
  await sendMessage(chatId, "✏️ Ketik nama project tujuan (baru):");
}

async function createTransferTargetAndConfirm(chatId, sourceId, projectName) {
  const { data: newProject, error } = await supabase
    .from("projects")
    .insert({ name: projectName, is_active: true })
    .select()
    .single();

  await clearSession(chatId);

  if (error || !newProject) {
    await sendMessage(chatId, "❌ Gagal membuat project baru: " + (error ? error.message : "unknown error"));
    return;
  }

  await sendMessage(chatId, `✅ Project <b>${projectName}</b> dibuat.`);
  await confirmTransfer(chatId, sourceId, newProject.id);
}

async function confirmTransfer(chatId, sourceId, targetId) {
  const [{ data: source }, { data: target }, { data: rows }] = await Promise.all([
    supabase.from("projects").select("*").eq("id", sourceId).single(),
    supabase.from("projects").select("*").eq("id", targetId).single(),
    supabase.from("material_usage").select("*").eq("project_id", sourceId),
  ]);

  if (!source || !target) {
    await sendMessage(chatId, "❌ Project tidak ditemukan.");
    return;
  }

  const usageRows = rows || [];
  if (usageRows.length === 0) {
    await sendMessage(chatId, `ℹ️ Tidak ada data pemakaian di project <b>${source.name}</b> untuk dipindahkan.`);
    await showProjectMenu(chatId);
    return;
  }

  const summaryMap = {};
  usageRows.forEach((r) => {
    if (!summaryMap[r.material_name]) summaryMap[r.material_name] = { total: 0, unit: r.unit };
    summaryMap[r.material_name].total += parseFloat(r.quantity);
  });

  let msg = `🔀 <b>Konfirmasi Pindahkan Data</b>\n\n`;
  msg += `Dari: <b>${source.name}</b>\nKe: <b>${target.name}</b>\n\n`;
  msg += `📦 <b>${usageRows.length} entri</b> akan dipindahkan:\n`;
  Object.entries(summaryMap).forEach(([name, info]) => {
    msg += `• ${name}: ${info.total.toFixed(1)} ${info.unit}\n`;
  });
  msg += `\nProject <b>${source.name}</b> tetap ada (kosong) untuk menampung pemakaian berikutnya.`;

  await sendInlineKeyboard(chatId, msg, [
    [
      { text: "✅ Ya, Pindahkan", callback_data: `transferok_${sourceId}_${targetId}` },
      { text: "❌ Batal", callback_data: "menu_project" },
    ],
  ]);
}

async function executeTransfer(chatId, sourceId, targetId) {
  const [{ data: source }, { data: target }] = await Promise.all([
    supabase.from("projects").select("*").eq("id", sourceId).single(),
    supabase.from("projects").select("*").eq("id", targetId).single(),
  ]);
  if (!source || !target) return sendMessage(chatId, "❌ Project tidak ditemukan.");

  const { data: updated, error } = await supabase
    .from("material_usage")
    .update({ project_id: targetId, project_name: target.name })
    .eq("project_id", sourceId)
    .select();

  if (error) {
    await sendMessage(chatId, "❌ Gagal memindahkan data: " + error.message);
    return;
  }

  await sendMessage(
    chatId,
    `✅ <b>${(updated || []).length} entri</b> berhasil dipindahkan dari <b>${source.name}</b> ke <b>${target.name}</b>.`
  );
  await showProjectMenu(chatId);
}

// ============================================================
// Generate Excel report pemakaian bahan untuk satu project
// (dipakai saat project dinonaktifkan)
// ============================================================
async function generateProjectUsageExcel(projectId) {
  const { data: rows, error } = await supabase
    .from("material_usage")
    .select("*")
    .eq("project_id", projectId)
    .order("report_date", { ascending: true });

  if (error) throw new Error(error.message);

  const usageRows = rows || [];

  // --- Sheet Ringkasan: total per bahan ---
  const summaryMap = {};
  usageRows.forEach((r) => {
    if (!summaryMap[r.material_name]) summaryMap[r.material_name] = { total: 0, unit: r.unit };
    summaryMap[r.material_name].total += parseFloat(r.quantity);
  });

  const summaryData = [["Nama Bahan", "Total Pemakaian", "Satuan"]];
  Object.entries(summaryMap).forEach(([name, info]) => {
    summaryData.push([name, Number(info.total.toFixed(3)), info.unit]);
  });

  const wsSummary = XLSX.utils.aoa_to_sheet(summaryData);
  wsSummary["!cols"] = [{ wch: 32 }, { wch: 18 }, { wch: 10 }];

  // --- Sheet Detail: semua transaksi ---
  const detailData = [["Tanggal", "Nama Bahan", "Jumlah", "Satuan", "Operator", "Dilaporkan (WIB)"]];
  usageRows.forEach((r) => {
    detailData.push([
      r.report_date,
      r.material_name,
      Number(parseFloat(r.quantity).toFixed(3)),
      r.unit,
      r.operator_name,
      new Date(r.reported_at).toLocaleString("id-ID", { timeZone: "Asia/Jakarta" }),
    ]);
  });

  const wsDetail = XLSX.utils.aoa_to_sheet(detailData);
  wsDetail["!cols"] = [{ wch: 12 }, { wch: 32 }, { wch: 12 }, { wch: 10 }, { wch: 20 }, { wch: 20 }];

  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, wsSummary, "Ringkasan");
  XLSX.utils.book_append_sheet(wb, wsDetail, "Detail");

  const buffer = XLSX.write(wb, { type: "buffer", bookType: "xlsx" });
  return { buffer, count: usageRows.length };
}

// ============================================================
// Helper: Jakarta date
// ============================================================
function getJakartaDate() {
  return new Date().toLocaleDateString("sv-SE", { timeZone: "Asia/Jakarta" });
}

function getDateOffset(days) {
  const d = new Date();
  d.setDate(d.getDate() + days);
  return d.toLocaleDateString("sv-SE", { timeZone: "Asia/Jakarta" });
}

function isValidDate(dateStr) {
  const regex = /^\d{4}-\d{2}-\d{2}$/;
  if (!regex.test(dateStr)) return false;
  return !isNaN(new Date(dateStr).getTime());
}

// ============================================================
// DAILY REPORT — kirim ke grup setiap pagi
// ============================================================
async function sendDailyReport() {
  const groupId = process.env.REPORT_GROUP_ID;
  if (!groupId) {
    console.log("REPORT_GROUP_ID not set, skipping daily report");
    return { success: false, reason: "No REPORT_GROUP_ID" };
  }

  // Ambil data kemarin
  const yesterday = getDateOffset(-1);

  const { data, error } = await supabase
    .from("material_usage")
    .select("*")
    .eq("report_date", yesterday)
    .neq("project_name", "RESTOCK")
    .order("project_name");

  // Hitung stok untuk alert
  const { data: allUsage } = await supabase
    .from("material_usage").select("material_id, quantity");

  const usageMap = {};
  if (allUsage) {
    allUsage.forEach((row) => {
      if (!usageMap[row.material_id]) usageMap[row.material_id] = 0;
      usageMap[row.material_id] += parseFloat(row.quantity);
    });
  }

  // === BANGUN PESAN REPORT ===
  let message = `📋 <b>LAPORAN PEMAKAIAN BAHAN APT</b>\n`;
  message += `⏰ ${new Date().toLocaleString("id-ID", { timeZone: "Asia/Jakarta" })}\n`;
  message += `${"─".repeat(30)}\n\n`;

  // Ambil SEMUA data pemakaian (kumulatif dari awal)
  const { data: allData } = await supabase
    .from("material_usage")
    .select("*")
    .neq("project_name", "RESTOCK")
    .neq("project_name", "ADJUSTMENT")
    .order("project_name");

  // Ambil semua project aktif
  const { data: projects } = await supabase
    .from("projects").select("*").eq("is_active", true).order("name");

  message += `🏗️ <b>TOTAL PEMAKAIAN PER PROJECT</b>\n\n`;

  if (projects && projects.length > 0) {
    // Group semua data by project
    const byProject = {};
    if (allData) {
      allData.forEach((row) => {
        const pid = row.project_id;
        if (!byProject[pid]) byProject[pid] = {};
        if (!byProject[pid][row.material_name]) byProject[pid][row.material_name] = { total: 0, unit: row.unit };
        byProject[pid][row.material_name].total += parseFloat(row.quantity);
      });
    }

    for (const proj of projects) {
      message += `▸ <b>${proj.name}</b>\n`;
      const materials = byProject[proj.id];
      if (materials && Object.keys(materials).length > 0) {
        for (const [matName, info] of Object.entries(materials)) {
          message += `   📦 ${matName}: ${info.total.toFixed(1)} ${info.unit}\n`;
        }
      } else {
        message += `   <i>Belum ada pemakaian</i>\n`;
      }
      message += "\n";
    }
  } else {
    message += `ℹ️ Belum ada project aktif.\n\n`;
  }

  // === ALERT STOK RENDAH ===
  const lowStockItems = [];
  for (const m of MATERIAL_LIST) {
    const used = usageMap[m.id] || 0;
    const current = m.initial_stock - used;
    if (current <= m.min_stock) {
      lowStockItems.push({ name: m.name, current, unit: m.unit, min: m.min_stock });
    }
  }

  if (lowStockItems.length > 0) {
    message += `${"─".repeat(30)}\n`;
    message += `🚨 <b>PERLU SEGERA DIBELI (${lowStockItems.length} item)</b>\n\n`;
    lowStockItems.forEach((item) => {
      message += `🔴 <b>${item.name}</b>\n`;
      message += `   Stok: ${item.current.toFixed(1)} ${item.unit} (min: ${item.min} ${item.unit})\n\n`;
    });
  } else {
    message += `${"─".repeat(30)}\n`;
    message += `🟢 Semua stok bahan aman.\n`;
  }

  message += `\n<i>— APT Material Bot</i>`;

  await sendMessage(groupId, message);

  // Kirim juga status stok LENGKAP (semua bahan, bukan cuma yang di bawah minimum)
  // supaya admin bisa langsung cek buat kebutuhan permintaan pembelian
  await sendLongMessage(groupId, buildStockStatusMessage(usageMap));
  return { success: true, date: getJakartaDate() };
}

// ============================================================
// Helper: hitung stok saat ini
// ============================================================
async function getCurrentStock(materialId) {
  const material = getMaterialById(materialId);
  if (!material) return 0;

  const { data } = await supabase
    .from("material_usage").select("quantity").eq("material_id", materialId);
  const totalUsed = data ? data.reduce((sum, r) => sum + parseFloat(r.quantity), 0) : 0;
  return material.initial_stock - totalUsed;
}

// ============================================================
// ADJUSTMENT STOK - Master only
// Flow: pilih supplier → pilih bahan (lihat stok) → input stok fisik → sistem koreksi
// ============================================================
async function startAdjustment(chatId) {
  const buttons = SUPPLIERS.map((s) => [
    { text: s.name, callback_data: `adjsup_${s.id}` },
  ]);
  buttons.push([{ text: "🏠 Menu Utama", callback_data: "menu_utama" }]);

  await sendInlineKeyboard(
    chatId,
    "🔧 <b>Adjustment Stok</b>\n\nPilih supplier untuk koreksi stok:",
    buttons
  );
}

async function showAdjustMaterials(chatId, supplierId) {
  const materials = getMaterialsBySupplier(supplierId);
  const supplier = SUPPLIERS.find((s) => s.id === supplierId);

  let message = `🔧 <b>Adjustment — ${supplier.name}</b>\n\nPilih bahan:\n\n`;

  const buttons = [];
  for (const m of materials) {
    const current = await getCurrentStock(m.id);
    const icon = current <= m.min_stock ? "🔴" : "🟢";
    message += `${icon} ${m.name}: ${current.toFixed(1)} ${m.unit}\n`;
    buttons.push([{
      text: `${icon} ${m.name} (${current.toFixed(1)} ${m.unit})`,
      callback_data: `adjmat_${m.id}`,
    }]);
  }

  buttons.push([
    { text: "🔙 Pilih Supplier", callback_data: "menu_adjust" },
    { text: "🏠 Menu Utama", callback_data: "menu_utama" },
  ]);

  await sendInlineKeyboard(chatId, message, buttons);
}

async function askAdjustQuantity(chatId, materialId) {
  const material = getMaterialById(materialId);
  if (!material) return;

  const currentStock = await getCurrentStock(materialId);

  await saveSession(chatId, {
    step: "input_adjust",
    material_id: materialId,
    material_name: material.name,
    material_unit: material.unit,
    current_stock: currentStock,
  });

  await sendMessage(chatId,
    `🔧 <b>Adjustment: ${material.name}</b>\n\n` +
    `📊 Stok di sistem: <b>${currentStock.toFixed(1)} ${material.unit}</b>\n\n` +
    `Ketik stok fisik sebenarnya sekarang:\n` +
    `(contoh: <code>150</code>)`
  );
}

async function confirmAdjustment(chatId, physicalStock, userName) {
  const session = await getSession(chatId);
  if (!session) return;

  if (!session.material_id || session.current_stock === undefined) {
    await sendMessage(
      chatId,
      "⚠️ Sesi input sudah kedaluwarsa atau kepakai untuk aksi lain di tengah jalan. Silakan ulangi dari awal."
    );
    await clearSession(chatId);
    return;
  }

  const currentStock = session.current_stock;
  const difference = physicalStock - currentStock; // positif = tambah, negatif = kurang

  if (difference === 0) {
    await sendMessage(chatId, `✅ Stok <b>${session.material_name}</b> sudah sesuai, tidak perlu adjustment.`);
    await clearSession(chatId);
    return;
  }

  // Simpan adjustment sebagai entry khusus
  const { error } = await supabase.from("material_usage").insert({
    material_id: session.material_id,
    material_name: session.material_name,
    quantity: -difference, // negatif dari difference karena rumus: initial - usage = stock
    unit: session.material_unit,
    supplier: "adjustment",
    project_id: null,
    project_name: "ADJUSTMENT",
    order_note: `Koreksi stok: ${currentStock.toFixed(1)} → ${physicalStock}`,
    operator_id: String(MASTER_CHAT_ID),
    operator_name: userName,
    reported_at: new Date().toISOString(),
    report_date: getJakartaDate(),
  });

  if (error) {
    await sendMessage(chatId, "❌ Gagal: " + error.message);
  } else {
    const arrow = difference > 0 ? "📈" : "📉";
    await sendMessage(chatId,
      `✅ <b>Adjustment Berhasil!</b>\n\n` +
      `📦 <b>${session.material_name}</b>\n` +
      `📊 Stok lama: ${currentStock.toFixed(1)} ${session.material_unit}\n` +
      `${arrow} Stok baru: <b>${physicalStock} ${session.material_unit}</b>\n` +
      `🔧 Selisih: ${difference > 0 ? "+" : ""}${difference.toFixed(1)} ${session.material_unit}`
    );
  }

  await sendInlineKeyboard(chatId, "Adjustment bahan lain?", [
    [
      { text: "🔧 Adjustment lagi", callback_data: "menu_adjust" },
      { text: "🏠 Menu Utama", callback_data: "menu_utama" },
    ],
  ]);

  await clearSession(chatId);
}

// ============================================================
// ADJUSTMENT VIA UPLOAD EXCEL (Master only)
// Flow: master pilih menu → bot minta upload file → parse Excel →
// cocokkan nama bahan → hitung selisih → insert entry ADJUSTMENT
// ============================================================

// Kirim pesan panjang secara aman (batas Telegram ~4096 karakter)
async function sendLongMessage(chatId, text) {
  const LIMIT = 3500;
  if (text.length <= LIMIT) {
    await sendMessage(chatId, text);
    return;
  }
  const lines = text.split("\n");
  let chunk = "";
  for (const line of lines) {
    if ((chunk + line + "\n").length > LIMIT) {
      await sendMessage(chatId, chunk);
      chunk = "";
    }
    chunk += line + "\n";
  }
  if (chunk.trim()) await sendMessage(chatId, chunk);
}

function normalizeName(s) {
  return String(s || "").toLowerCase().replace(/\s+/g, " ").trim();
}

// Cocokkan nama bahan dari Excel ke MATERIAL_LIST (exact match dulu, lalu fallback contains)
function findMaterialByName(rawName) {
  const norm = normalizeName(rawName);
  if (!norm) return null;

  let m = MATERIAL_LIST.find((x) => normalizeName(x.name) === norm);
  if (m) return m;

  m = MATERIAL_LIST.find(
    (x) => normalizeName(x.name).includes(norm) || norm.includes(normalizeName(x.name))
  );
  return m || null;
}

async function startAdjustExcel(chatId) {
  await saveSession(chatId, { step: "waiting_adjust_excel" });
  await sendMessage(
    chatId,
    `📥 <b>Upload Excel Adjustment Stok</b>\n\n` +
      `Kirim file <b>.xlsx</b> hasil isian template "Adjustment Stok" (kolom Nama Bahan, Stok Fisik, Keterangan).\n\n` +
      `• Baris tanpa "Stok Fisik" akan dilewati (dianggap tidak perlu adjustment)\n` +
      `• Baris contoh di template otomatis diabaikan\n\n` +
      `Ketik /batal untuk membatalkan.`
  );
}

async function processAdjustmentExcel(chatId, fileBuffer, userName) {
  let workbook;
  try {
    workbook = XLSX.read(fileBuffer, { type: "buffer" });
  } catch (err) {
    await sendMessage(chatId, "❌ File tidak bisa dibaca. Pastikan file .xlsx yang valid dan belum rusak.");
    await clearSession(chatId);
    return;
  }

  const sheetName =
    workbook.SheetNames.find((n) => n.toLowerCase().includes("adjust")) || workbook.SheetNames[0];
  const sheet = workbook.Sheets[sheetName];
  const rows = XLSX.utils.sheet_to_json(sheet, { header: 1, defval: "" });

  // Cari baris header dengan mendeteksi kolom "Nama Bahan" & "Stok Fisik"
  let headerRowIdx = -1;
  let colName = -1;
  let colStokFisik = -1;
  let colKeterangan = -1;

  for (let i = 0; i < rows.length; i++) {
    const row = rows[i];
    let nameCol = -1, stokCol = -1, ketCol = -1;
    for (let j = 0; j < row.length; j++) {
      const cell = String(row[j] || "").toLowerCase();
      if (cell.includes("nama bahan")) nameCol = j;
      if (cell.includes("stok fisik")) stokCol = j;
      if (cell.includes("keterangan")) ketCol = j;
    }
    if (nameCol !== -1 && stokCol !== -1) {
      headerRowIdx = i;
      colName = nameCol;
      colStokFisik = stokCol;
      colKeterangan = ketCol;
      break;
    }
  }

  if (headerRowIdx === -1) {
    await sendMessage(
      chatId,
      `❌ Format Excel tidak dikenali.\n\nPastikan file menggunakan template "Adjustment Stok" dengan kolom <b>Nama Bahan</b> dan <b>Stok Fisik</b>.`
    );
    await clearSession(chatId);
    return;
  }

  const adjusted = [];
  const skippedNoChange = [];
  const notFound = [];
  const inserts = [];

  for (let i = headerRowIdx + 1; i < rows.length; i++) {
    const row = rows[i];
    if (!row || row.length === 0) continue;

    const rawName = row[colName];
    const rawKeterangan = colKeterangan !== -1 ? row[colKeterangan] : "";
    if (!rawName) continue;
    if (String(rawKeterangan).trim().toLowerCase().startsWith("contoh")) continue; // lewati baris contoh

    const rawStokFisik = row[colStokFisik];
    if (rawStokFisik === "" || rawStokFisik === null || rawStokFisik === undefined) continue; // tidak diisi = skip

    const stokFisik = parseFloat(String(rawStokFisik).replace(",", "."));
    if (isNaN(stokFisik) || stokFisik < 0) {
      notFound.push(`${rawName} — stok fisik tidak valid: "${rawStokFisik}"`);
      continue;
    }

    const material = findMaterialByName(rawName);
    if (!material) {
      notFound.push(`${rawName} — nama bahan tidak dikenali`);
      continue;
    }

    const currentStock = await getCurrentStock(material.id);
    const difference = stokFisik - currentStock;

    if (Math.abs(difference) < 0.0001) {
      skippedNoChange.push(`${material.name}: ${currentStock.toFixed(1)} ${material.unit} (sudah sesuai)`);
      continue;
    }

    inserts.push({
      material_id: material.id,
      material_name: material.name,
      quantity: -difference, // negatif dari difference: initial - usage = stock
      unit: material.unit,
      supplier: "adjustment",
      project_id: null,
      project_name: "ADJUSTMENT",
      order_note:
        `Koreksi via Excel: ${currentStock.toFixed(1)} → ${stokFisik}` +
        (rawKeterangan ? ` | ${String(rawKeterangan).trim()}` : ""),
      operator_id: String(MASTER_CHAT_ID),
      operator_name: userName,
      reported_at: new Date().toISOString(),
      report_date: getJakartaDate(),
    });

    const arrow = difference > 0 ? "📈" : "📉";
    adjusted.push(
      `${arrow} <b>${material.name}</b>: ${currentStock.toFixed(1)} → <b>${stokFisik}</b> ${material.unit} (${difference > 0 ? "+" : ""}${difference.toFixed(1)})`
    );
  }

  if (inserts.length > 0) {
    const { error } = await supabase.from("material_usage").insert(inserts);
    if (error) {
      await sendMessage(chatId, "❌ Gagal menyimpan adjustment ke database. Error: " + error.message);
      await clearSession(chatId);
      return;
    }
  }

  let summary = `📥 <b>Hasil Proses Excel Adjustment</b>\n\n`;
  if (adjusted.length > 0) {
    summary += `✅ <b>${adjusted.length} bahan di-adjust:</b>\n` + adjusted.join("\n") + `\n\n`;
  } else {
    summary += `ℹ️ Tidak ada bahan yang perlu di-adjust.\n\n`;
  }
  if (skippedNoChange.length > 0) {
    summary += `➖ <b>${skippedNoChange.length} bahan sudah sesuai</b> (dilewati)\n\n`;
  }
  if (notFound.length > 0) {
    summary +=
      `⚠️ <b>${notFound.length} baris perlu dicek manual:</b>\n` +
      notFound.map((n) => `• ${n}`).join("\n") +
      `\n\n`;
  }

  await sendLongMessage(chatId, summary);
  await sendInlineKeyboard(chatId, "Selesai memproses.", [
    [
      { text: "📥 Upload lagi", callback_data: "menu_adjust_excel" },
      { text: "🏠 Menu Utama", callback_data: "menu_utama" },
    ],
  ]);

  await clearSession(chatId);
}

module.exports = {
  showMainMenu, startReport, showMaterials, askQuantity, askDate,
  askProject, confirmAndSave, showTodayStatus, showHistory, showStockStatus,
  showProjectMenu, startAddProject, saveProject, showToggleProjects, toggleProject,
  startTransferProject, showTransferTargets, startTransferNewProject,
  createTransferTargetAndConfirm, confirmTransfer, executeTransfer,
  startRestock, askRestockQuantity, confirmRestock,
  showRecentEntries, deleteEntry,
  showProjectReport, showProjectDetail,
  sendDailyReport,
  startAdjustment, showAdjustMaterials, askAdjustQuantity, confirmAdjustment,
  startAdjustExcel, processAdjustmentExcel,
  getSession, clearSession, saveSession, isValidDate, isMaster,
};
